...
def preprocess_dataset(dataset: Dataset, tokenizer: BertTokenizerFast) \
    ...
    """
    def tokenize_line(row):
        return tokenizer(row["text"], padding="max_length", max_length=512, truncation=True)

    return dataset.map(tokenize_line, batched=True)
...
def init_model(trial: Any, model_name: str, use_bitfit: bool = False) -> \
    ...
    if use_bitfit:
        for name, param in model.named_parameters():
            if 'bias' not in name:
                param.requires_grad = False # freeze the param
    return model
